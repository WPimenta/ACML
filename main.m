...
% for option=1:size(options{bestInfGainNode}, 2)
%     [yes, no] = GetNumYesNo(all_data, options{bestInfGainNode}{option}, bestInfGainNode, all_results);
%     disp([options{bestInfGainNode}{option} ': Yes = ' num2str(yes) ', No = ' num2str(no)]);
%     if (yes == 0 && no == 0)
%         [specificRecords, specificResults] = GetRecordsForSubOption(all_data, bestInfGainNode, options{bestInfGainNode}{option}, all_results);
%         numPositive = 0;
%         numNegative = 0;
